#!/bin/bash
# Chained pipeline for ITERATION-2 / 005_dlv3plus_mobilevit_xs_512:
#   1. train  → 2. evaluate (Strategy A)  → 3. space_grade (--input-hw 512)
#
# Runs on GPU 1 via CUDA_VISIBLE_DEVICES=1 to allow parallel execution alongside
# 004_dlv3plus_256 (which is on GPU 0). Same nohup pattern as 003/004.
#
# Encoder choice: tu-mobilevit_xs (hybrid CNN+transformer, 2.92 M total params with
# DLV3+ decoder = 97.3% of the 3 M cap). First attention-based encoder on AI4Mars
# under the cap.

set -u
cd "$(dirname "$0")/../../.."   # → .
ROOT="$(pwd)"
EXP_ID="005_dlv3plus_mobilevit_xs_512"
EXP_DIR="ITERATION-2/experiments/${EXP_ID}"
VENV="${ROOT}/.venv/bin/python"
STATUS_DIR="${EXP_DIR}"

# Bind to GPU 1 — keep this entire pipeline (training + space_grade GPU latency)
# on the same device.
...
{
  echo
  echo "[STAGE 1/3] Training DeepLabV3+ + tu-mobilevit_xs @ 512x512 (GPU 1)"
  echo "  start: $(date_iso)"
} >> "${PIPELINE_LOG}"
...
"${VENV}" DeepLabV3Plus_training_v2.py \
    ...
    --input-hw 512 \
    --description "DeepLabV3+ + MobileViT-XS at 512x512 (paradigm-distinct cross-encoder experiment vs 002/003). 2.92 M params = 97.3% of 3 M cap. First attention-based encoder under the space-grade cap on AI4Mars." \
