"""Trainer for the AI4Mars DeepLabV3+ pipeline (MAIN_ITERATION_V2).

V2 trainer using DeepLabV3+ as the decoder family and MobileNetV4-Conv-Small
as the encoder by default — the architecture chosen in ITERATION-2 (cell 13)
for best-converged train/val Big Rock IoU. Total parameter count = 3.00 M
trainable (exactly at the 3 M space-grade cap; BN buffers add ~27 k that don't
count against the cap).

Sister file: `DLV3+_MNV3_small_trainer.py` (the v1 MAIN_ITERATION trainer with
MobileNetV3-Small encoder). This file is *identical* in every respect except
two defaults: `--encoder` and `--exp-root`. That isolation is intentional —
v2 runs are bit-identical to v1 runs in loss math, optimizer math, data
ordering, seeding, and metric computation, so a v1→v2 row pair differs only
in encoder choice. All loss knobs (focal-gamma, focal-tversky-alpha,
tversky-alpha/beta, ftl-gamma, etc.) and the per-epoch checkpoint workflow
work the same way.

Feature set (unchanged from v1 trainer):
- Argparse with `--exp-id` (required) to scope outputs to <exp-root>/<exp-id>/
- Choice of loss (`--loss ce | focal | focal_dice | focal_tversky | ftl`)
...
def parse_args():
    ap = argparse.ArgumentParser(
        description="Train DeepLabV3+ + MobileNetV4-Conv-Small on AI4Mars MSL NCAM "
                    "for MAIN_ITERATION_V2. Defaults mirror the v1 trainer's "
                    "(DLV3+_MNV3_small_trainer.py) protocol exactly except for "
                    "encoder choice and exp-root, so v1→v2 comparisons isolate "
                    "the encoder lever."
    )
    ap.add_argument("--exp-id", required=True,
                    help="Experiment ID. Outputs saved under <exp-root>/<exp-id>/.")
    ap.add_argument("--exp-root", default="MAIN_ITERATION_V2/experiments",
                    help="Parent folder for experiment outputs. Defaults to the "
                         "v2 experiments folder (mirrors v1's MAIN_ITERATION/experiments).")
    ...
    ap.add_argument("--encoder", default="tu-mobilenetv4_conv_small",
                    help="smp encoder name. Default tu-mobilenetv4_conv_small puts "
                         "total trainable params at ~3.00 M (exactly at the 3 M "
                         "space-grade cap). This is the iter-2 winner encoder.")
